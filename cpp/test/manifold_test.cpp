...
template <typename T>
int NumUnique(const VecView<T> in) {
  std::set<int> unique;
  for (const T& v : in) {
    unique.emplace(v);
  }
...
TEST(Manifold, FaceIDRoundTrip) {
  ...
  ASSERT_EQ(
      NumUnique(manifold::VecView(inGL.faceID.data(), inGL.faceID.size())), 6);
  inGL.faceID = {3, 3, 3, 3, 3, 3, 5, 5, 5, 5, 5, 5};

  const Manifold cube2(inGL);
  const MeshGL outGL = cube2.GetMeshGL();
  ASSERT_EQ(
      NumUnique(manifold::VecView(outGL.faceID.data(), outGL.faceID.size())),
      2);
}
