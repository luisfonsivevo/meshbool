...
public:
  ...
  inline static Manifold Cube(vec3 size = vec3(1.0), bool center = false) {
    ...
    manifold_cube(ptr, size.x, size.y, size.z, center);
  ...
  inline static Manifold Cylinder(double height, double radiusLow,
                                  ...
                                  bool center = false) {
    auto ptr = malloc(manifold_manifold_size());
    manifold_cylinder(ptr, height, radiusLow, radiusHigh, circularSegments,
                      center);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  inline static Manifold Sphere(double radius, int circularSegments = 0) {
    auto ptr = malloc(manifold_manifold_size());
    manifold_sphere(ptr, radius, circularSegments);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  ...
  inline size_t NumEdge() const {
    return manifold_num_edge(this->pNode_.get());
  }
  inline size_t NumTri() const { return manifold_num_tri(this->pNode_.get()); }
  inline size_t NumProp() const {
    return manifold_num_prop(this->pNode_.get());
  }
  // inline size_t NumPropVert() const {
  //   return manifold_num_prop(this->pNode_.get());
  // }
  ...
  inline int OriginalID() const {
    return manifold_original_id(this->pNode_.get());
  }
  inline Manifold AsOriginal() const {
    auto ptr = malloc(manifold_manifold_size());
   ...
   */
  ///@{
  inline Manifold Translate(vec3 p) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_translate(ptr, this->pNode_.get(), p.x, p.y, p.z);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  inline Manifold Scale(vec3 s) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_scale(ptr, this->pNode_.get(), s.x, s.y, s.z);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  inline Manifold Rotate(double xDegrees, double yDegrees = 0.0,
                         double zDegrees = 0.0) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_rotate(ptr, this->pNode_.get(), xDegrees, yDegrees, zDegrees);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  // Manifold Mirror(vec3) const;
  inline Manifold Transform(const mat3x4 &m) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_transform(ptr, this->pNode_.get(), m.x.x, m.x.y, m.x.z, m.y.x,
                       m.y.y, m.y.z, m.z.x, m.z.y, m.z.z, m.w.x, m.w.y, m.w.z);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  ...
  // Add (Union)
  inline Manifold operator+(const Manifold &other) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_boolean(ptr, this->pNode_.get(), other.pNode_.get(), MANIFOLD_ADD);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  inline Manifold &operator+=(const Manifold &Q) {
    *this = *this + Q;
    return *this;
  }
  // Subtract (Difference)
  inline Manifold operator-(const Manifold &other) const {
    auto ptr = malloc(manifold_manifold_size());
    manifold_boolean(ptr, this->pNode_.get(), other.pNode_.get(),
                     MANIFOLD_SUBTRACT);
    return Manifold(static_cast<ManifoldManifold *>(ptr));
  }
  inline Manifold &operator-=(const Manifold &Q) {
