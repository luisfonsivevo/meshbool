...
use crate::shared::{get_axis_aligned_projection, next_halfedge, Halfedge};
use crate::utils::ccw;
...
fn is_01_longest(v0: Point2<f64>, v1: Point2<f64>, v2: Point2<f64>) -> bool
{
	...
	let mut l = [0.0; 3];
	for i in 0..3 { l[i] = e[i].magnitude_squared(); }
	l[0] > l[1] && l[0] > l[2]
}
...
impl<'a> Pred for ShortEdge<'a>
{
	fn call(&self, edge: usize) -> bool
	{
		...
		// Flag short edges
		let delta = self.r#impl.vert_pos[half.end_vert as usize] - self.r#impl.vert_pos[half.start_vert as usize];
...
impl<'a> Pred for FlagEdge<'a>
{
	...
	{
		let half = &self.r#impl.halfedge[edge];
		if half.paired_halfedge < 0 || half.start_vert < self.first_new_vert { return false; }
...
impl<'a> Pred for SwappableEdge<'a>
{
	fn call(&self, edge: usize) -> bool
	{
		...
		if half.paired_halfedge < 0 { return false; }
		if half.start_vert < self.first_new_vert &&
				half.end_vert < self.first_new_vert &&
				self.r#impl.halfedge[next_halfedge(edge) as usize].end_vert < self.first_new_vert &&
				self.r#impl.halfedge[next_halfedge(half.paired_halfedge) as usize].end_vert < self.first_new_vert
		{
			return false;
		}
		...
		for i in 0..3
		{
			v[i] = projection * self.r#impl.vert_pos[self.r#impl.halfedge[tri_edge[i] as usize].start_vert as usize];
		}
...
impl FlagStore
{
		...
		F: FnMut(&mut Impl, usize),
	{
		for i in 0..n
		{
			if pred.call(i) { self.s.push(i); }
		}
		
		for &i in &self.s { f(pred.get_impl(), i); }
	}
...
impl Impl
{
	...
	{
		if self.halfedge.is_empty() { return; }
	...
	fn swap_degenerates(&mut self, first_new_vert: i32)
	{
		...
		s.run(nb_edges, se, |myself, i|
		{
			num_flagged += 1;
			tag += 1;
			myself.recursive_edge_swap(i as i32, &mut tag, &mut visited, &mut edge_swap_stack, &mut scratch_buffer);
			while !edge_swap_stack.is_empty()
			{
				let last = edge_swap_stack.pop().unwrap();
				myself.recursive_edge_swap(last, &mut tag, &mut visited, &mut edge_swap_stack, &mut scratch_buffer);
			}
		});
	}
	...
	fn dedupe_edge(&mut self, edge: i32)
	{
		...
		while current != edge
		{
			...
			if vert == start_vert
			{
				...
				self.pair_up(new_halfedge + 2, self.halfedge[current as usize].paired_halfedge);
				self.pair_up(new_halfedge + 1, current);
				if self.mesh_relation.tri_ref.len() > 0
				{
					self.mesh_relation.tri_ref.push(self.mesh_relation.tri_ref[old_face as usize]);
				}
				...
				self.pair_up(new_halfedge + 2, self.halfedge[opposite as usize].paired_halfedge);
	...
	fn form_loop(&mut self, current: i32, end: i32)
	{
		...
		self.vert_pos.push(self.vert_pos[self.halfedge[current as usize].start_vert as usize]);
		let end_vert = self.vert_pos.len() as i32;
		self.vert_pos.push(self.vert_pos[self.halfedge[current as usize].end_vert as usize]);
	...
	{
		if self.halfedge[tri_edge[1] as usize].paired_halfedge == -1 { return; }
	...
	fn remove_if_folded(&mut self, edge: i32)
	{
		...
		if self.halfedge[tri0_edge[1] as usize].paired_halfedge == -1 { return; }
		if self.halfedge[tri0_edge[1] as usize].end_vert == self.halfedge[tri1_edge[1] as usize].end_vert
		{
			if self.halfedge[tri0_edge[1] as usize].paired_halfedge == tri1_edge[2]
			{
				if self.halfedge[tri0_edge[2] as usize].paired_halfedge == tri1_edge[1]
				{
					for i in 0..3
					{
						self.vert_pos[self.halfedge[tri0_edge[i] as usize].start_vert as usize] = Point3::new(f64::NAN, f64::NAN, f64::NAN);
					}
				}
				else
				{
					self.vert_pos[self.halfedge[tri0_edge[1] as usize].start_vert as usize] = Point3::new(f64::NAN, f64::NAN, f64::NAN);
				}
			}
			else
			{
				if self.halfedge[tri0_edge[2] as usize].paired_halfedge == tri1_edge[1]
				{
					self.vert_pos[self.halfedge[tri1_edge[1] as usize].start_vert as usize] = Point3::new(f64::NAN, f64::NAN, f64::NAN);
				}
			}
			
			self.pair_up(self.halfedge[tri0_edge[1] as usize].paired_halfedge,
					self.halfedge[tri1_edge[2] as usize].paired_halfedge);
			self.pair_up(self.halfedge[tri0_edge[2] as usize].paired_halfedge,
	...
	fn collapse_edge(&mut self, edge: i32, edges: &mut Vec<i32>) -> bool
	{
		...
		if to_remove.paired_halfedge < 0 { return false; }
		...
		if !short_edge
		{
			...
			let mut ref_check = self.mesh_relation.tri_ref[(to_remove.paired_halfedge / 3) as usize];
			...
			while current != tri1_edge[0]
			{
				...
				if !r#ref.same_face(&ref_check)
				{
					...
					if r#ref.mesh_id != old_ref.mesh_id ||
							...
							self.face_normal[(to_remove.paired_halfedge / 3) as usize].dot(&self.face_normal[tri]) < -0.5
					{
						...
						if ccw(projection * p_last, projection * p_old, projection * p_new, self.epsilon) != 0
						{
							return false;
						}
					}
				...
				if ccw(projection * p_next, projection * p_last, projection * p_new, self.epsilon) < 0
				{
					return false;
				}
		...
		while current != tri0_edge[2]
		{
			...
			if self.num_prop() > 0
			{
				...
				{
					self.halfedge[current as usize].prop_vert = self.halfedge[next_halfedge(edge) as usize].prop_vert;
				}
				else if self.mesh_relation.tri_ref[tri].same_face(&self.mesh_relation.tri_ref[tri1])
				{
	...
	)
	{
		...
		if visited[edge as usize] == *tag && visited[pair as usize] == *tag { return; }
		...
		for i in 0..3
		{
			v[i] = projection * self.vert_pos[self.halfedge[tri0_edge[i] as usize].start_vert as usize];
		}
		...
		let swap_edge = |myself: &mut Impl|
		{
			...
			myself.pair_up(tri0_edge[0], myself.halfedge[tri1_edge[2] as usize].paired_halfedge);
			myself.pair_up(tri1_edge[0], myself.halfedge[tri0_edge[2] as usize].paired_halfedge);
			...
			{
				myself.halfedge[tri0_edge[1] as usize].prop_vert = myself.halfedge[tri1_edge[0] as usize].prop_vert;
				myself.halfedge[tri0_edge[0] as usize].prop_vert = myself.halfedge[tri1_edge[2] as usize].prop_vert;
				myself.halfedge[tri0_edge[2] as usize].prop_vert = myself.halfedge[tri1_edge[2] as usize].prop_vert;
				...
				for p in 0..num_prop
				{
					myself.properties.push(a * myself.properties[num_prop * prop_idx0 + p] +
	...
	fn split_pinched_verts(&mut self)
	{
		...
		
		{
			...
			for i in 0..nb_edges
			{
				if halfedge_processed[i] { continue; }
	...
	fn dedupe_edges(&mut self)
	{
		loop
		{
			...
			let local_loop = |start: usize, end: usize, local: &mut Vec<bool>, results: &mut Vec<usize>|
			{
				...
				for i in start..end
				{
					if local[i] || self.halfedge[i].start_vert == -1 || self.halfedge[i].end_vert == -1
					{
						continue;
					}
					...
					self.for_vert(i as i32, |current|
					{
						...
						{
							let iter = end_verts.iter_mut().find(
									|pair| pair.0 == end_v);
					...
					self.for_vert(i as i32, |current|
					{
						...
						if end_vert_set.is_empty()
						{
							let iter = end_verts.iter().find(
									|pair| pair.0 == end_v).unwrap();
							
							if iter.1 != current { results.push(current as usize); }
						}
						else
						{
							let iter = *end_vert_set.get(&end_v).unwrap();
							if iter != current { results.push(current as usize); }
						}
					});
				}
			};
