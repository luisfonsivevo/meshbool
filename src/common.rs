...
impl AABB
{
	...
	{
		self.min.iter().all(|x| x.is_finite()) &&
				self.max.iter().all(|x| x.is_finite())
	}
}
...
impl Quality
{
	...
	{
		if Self::CIRCULAR_SEGMENTS > 0 { return Self::CIRCULAR_SEGMENTS; }
...
pub fn sun_acos(x: f64) -> f64
{
	...
	if ix >= 0x3ff00000
	{
		...
		{
			/* acos(1)=0, acos(-1)=pi */
			if (hx >> 31) != 0 { return 2.0 * PIO2_HI + 2.0_f64.powi(-120); }
			return 0.0;
		}
		return f64::NAN;
	}
