...
pub fn cube(size: Vector3<f64>, center: bool) -> Impl
{
	...
	[
		...
		if center { -size / 2.0 } else { Vector3::zeros() }
	]);
...
pub fn cylinder(height: f64, radius_low: f64, radius_high: f64, circular_segments: u32, center: bool) -> Impl
{
...
///Default {1, 1}.
pub fn extrude(cross_section: &Polygons, height: f64, mut n_divisions: u32, twist_degrees: f64, mut scale_top: Point2<f64>) -> Impl
{
	...
	for poly in cross_section
	{
		...
		for poly_vert in poly
		{
			...
			simple_indexed.push(PolyVert { pos: *poly_vert, idx });
			idx += 1;
		}
	...
	for i in 1..(n_divisions + 1)
	{
		...
		(
			phi.to_radians().cos(), -phi.to_radians().sin(),
			phi.to_radians().sin(), phi.to_radians().cos()
		);
	...
	if is_cone
	{
		for _ in 0..cross_section.len() // Duplicate vertex for Genus
		{
