...
use crate::polygon::{triangulate_idx, PolyVert, PolygonsIdx, SimplePolygonIdx};
use crate::shared::{get_axis_aligned_projection, Halfedge, TriRef};
use crate::utils::ccw;
...
fn assemble_halfedges(edges: &[Halfedge], start_halfedge_idx: i32) -> Vec<Vec<i32>>
{
	...
	loop
	{
		...
		{
			if vert_edge.is_empty() { break; }
			start_edge = vert_edge[0].1;
			this_edge = start_edge;
			polys.push(Vec::new());
		}
		
		polys.last_mut().unwrap().push(start_halfedge_idx + this_edge);
		let result = vert_edge.binary_search_by_key(&edges[this_edge as usize].end_vert, |&(vert, _)| vert);
...
impl Impl
{
	...
	pub(crate) fn face2tri(&mut self, face_edge: &[i32], halfedge_ref: &[TriRef], allow_convex: bool)
	{
		...
		{
			self.process_face(face_edge, &halfedge_ref, general_triangulation, |_, tri, normal, r|
			{
	...
	)
	{
		...
		else if num_edge == 4 //pair of triangles
		{
			let projection = get_axis_aligned_projection(normal);
			...
			let quad = &assemble_halfedges(&self.halfedge
			[
				face_edge[face] as usize..
				face_edge[face + 1] as usize
			], face_edge[face])[0];
			...
			{
				let diag0 =
						self.vert_pos[self.halfedge[quad[0] as usize].start_vert as usize] -
						self.vert_pos[self.halfedge[quad[2] as usize].start_vert as usize];
				let diag1 =
						self.vert_pos[self.halfedge[quad[1] as usize].start_vert as usize] -
