...
use crate::utils::{mat3, next3_usize, K_PRECISION};
...
pub fn get_axis_aligned_projection(normal: Vector3<f64>) -> Matrix2x3<f64>
{
	...
	if xyz_max < 0.0 { projection.row_mut(0).mul_assign(-1.0); }
	projection
}
...
pub fn get_barycentric(v: &Point3<f64>, tri_pos: &Matrix3<f64>, tolerance: f64) -> Vector3<f64>
{
	...
	if d2[long_side] < tol2 //point
	{
		return Vector3::new(1.0, 0.0, 0.0);
	}
	else if area2 > d2[long_side] * tol2 //triangle
	{
		for i in 0..3
		{
			let j = next3_usize(i);
			...
			uvw[i] = if area_2v < d2[i] * tol2 { 0.0 } else { cross_pv.dot(&cross_p) };
		}
	...
	else //line
	{
		...
		let alpha = (v - tri_pos.column(next_v)).coords.dot(&edges.column(long_side)) / d2[long_side];
		uvw[long_side] = 0.0;
