...
use crate::common::{OpType, OrderedF64, AABB};
use crate::r#impl::{Impl, MESH_ID_COUNTER};
use crate::parallel::{copy_if, exclusive_scan_transformed, gather, gather_transformed, inclusive_scan};
use crate::shared::{get_barycentric, Halfedge, TriRef};
...
impl<'a, const ATOMIC: bool> CountVerts<'a, ATOMIC>
{
	fn call(&mut self, i: usize)
	{
		if ATOMIC
		{
			unsafe { atomic_add_i32(&mut self.count[i / 3], self.inclusion[self.halfedges[i].start_vert as usize].abs()); }
		}
		else
		{
	...
	fn call(&mut self, idx: usize)
	{
		...
		if ATOMIC
		{
			unsafe
			{
				...
				atomic_add_i32(&mut self.count_p[(half.paired_halfedge / 3) as usize], inclusion);
			}
		}
		else
		{
...
) -> (Vec<i32>, Vec<i32>)
{
	...
	let
	(
		mut sides_per_face_p,
		mut sides_per_face_q
	...
	inclusive_scan(sides_per_face_pq.iter().map(
			|&x| if x > 0 { 1 } else { 0 }), &mut face_pq2r[1..]);
	let num_face_r = face_pq2r.pop().unwrap() as usize;
	
	unsafe { vec_resize_nofill(&mut out_r.face_normal, num_face_r); }
	...
	let next = copy_if(face_ids, &mut tmp_buffer,
			|v| v != usize::MAX);
	
	gather(&tmp_buffer[..next], &in_p.face_normal, &mut out_r.face_normal);
	...
	let end = next + copy_if(face_ids_q, &mut tmp_buffer[next..],
			&|v| v != usize::MAX);
	
	if invert_q
	{
		gather_transformed(&tmp_buffer[next..end], &in_q.face_normal,
				&mut out_r.face_normal[next..],
				|normal: Vector3<f64>| -normal);
	}
	else
	{
		gather(&tmp_buffer[next..end], &in_q.face_normal,
				&mut out_r.face_normal[next..]);
	}
...
)
{
	...
	for i in 0..p1q2.len()
	{
		...
		if !forward { mem::swap(&mut key_right.0, &mut key_right.1); }
		
		let mut key_left = (edge_p / 3, face_q);
		if !forward { mem::swap(&mut key_left.0, &mut key_left.1); }
		...
		for k in 0..3
		{
			let tuple = match k
			{
				...
				1 => (stable_direction ^ !forward, edges_new.entry(key_right).or_default()), //revert if not forward
				2 => (stable_direction ^ forward, edges_new.entry(key_left).or_default()),
				_ => unreachable!(),
			};
...
fn pair_up(edge_pos: &mut [EdgePos]) -> Vec<Halfedge>
{
	...
	// a heuristic.
	debug_assert!(edge_pos.len() % 2 == 0, "Non-manifold edge! Not an even number of points.");
...
)
{
	...
	for (&edge_p, edge_pos_p) in edges_p.iter_mut()
	{
		...
		{
			edge_pos: out_r.vert_pos[v_p2r[v_start] as usize].coords.dot(&edge_vec),
			vert: v_p2r[v_start],
...
)
{
	...
	{
		if !whole_halfedge_p[idx] { continue; }
		let mut halfedge = in_p.halfedge[idx];
		if !halfedge.is_forward() { continue; }
...
impl<'a> UpdateReference<'a>
{
	fn call(&self, tri_ref: &mut TriRef)
	{
		...
		*tri_ref = if pq
		...
		if !pq { tri_ref.mesh_id += self.offset_q; }
	}
}
...
fn update_reference(out_r: &mut Impl, in_p: &Impl, in_q: &Impl, invert_q: bool)
{
	...
	{
		out_r.mesh_relation.mesh_id_transform.entry(*pair.0).or_insert(*pair.1);
	}
	...
	for pair in &in_q.mesh_relation.mesh_id_transform
	{
		...
		out_r.mesh_relation.mesh_id_transform.entry(pair.0 + offset_q).or_insert(relation);
	}
}
...
impl<'a> Barycentric<'a>
{
	fn call(&mut self, tri: usize)
	{
		...
		if self.halfedge_r[3 * tri].start_vert < 0 { return; }
		
		let tri_pq = ref_pq.face_id;
		let pq = ref_pq.mesh_id == 0;
		let vert_pos = if pq { &self.vert_pos_p } else { &self.vert_pos_q };
		let halfedge = if pq { &self.halfedge_p } else { &self.halfedge_q };
		
		let mut tri_pos = Matrix3::default();
		for j in 0..3
		{
			*tri_pos.column_mut(j as usize) = *vert_pos[halfedge[(3 * tri_pq + j) as usize].start_vert as usize].deref();
		}
		
		for i in 0..3
		{
			let vert = self.halfedge_r[3 * tri + i].start_vert;
			self.uvw[3 * tri + i] = get_barycentric(&self.vert_pos_r[vert as usize], &tri_pos, self.epsilon);
		}
	}
}
...
fn create_properties(out_r: &mut Impl, in_p: &Impl, in_q: &Impl)
{
	...
	out_r.properties.reserve_exact(
			(out_r.num_vert() * num_prop).saturating_sub(out_r.properties.len()));
	let mut idx = 0;
	
	for tri in 0..num_tri
	{
		// Skip collapsed triangles
		if out_r.halfedge[3 * tri].start_vert < 0 { continue; }
		...
		for i in 0..3
		{
			...
			for p in 0..num_prop
			{
				...
				if p < old_num_prop
				{
					...
					for j in 0..3
					{
						old_props[j as usize] = properties[(old_num_prop * halfedge[(3 * r#ref.face_id + j) as usize].prop_vert + p) as usize];
					}
...
fn reorder_halfedges(halfedges: &mut [Halfedge])
{
	...
	for tri in 0..halfedges.len() / 3
	{
		...
		for i in 1..3
		{
			if face[i].start_vert < face[index].start_vert { index = i; };
		}
		for i in 0..3
		{
...
impl<'a> Boolean3<'a>
{
	...
	{
		debug_assert!((self.expand_p > 0.0) == (op == OpType::Add),
		...
		add_new_edge_verts(&mut edges_p, &mut edges_new, &self.p1q2, &i12, &v12_r, &self.in_p.halfedge, true, 0);
		add_new_edge_verts(&mut edges_q, &mut edges_new, &self.p2q1, &i21, &v21_r, &self.in_q.halfedge, false, self.p1q2.len());
		
		drop(v12_r);
		drop(v21_r);
		
		// Level 4
		let (face_edge, face_pq2r) =
				size_output(&mut out_r, &self.in_p, &self.in_q, &i03, &i30, i12, i21, &self.p1q2, &self.p2q1, invert_q);
