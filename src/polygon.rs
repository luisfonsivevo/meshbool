...
impl EarClip
{
	fn new(polys: &PolygonsIdx, epsilon: f64) -> EarClip
	{
		...
		{
			Self::clip_if_degenerate(v, &mut ret.polygon, &ret.polygon_range, &mut ret.triangles, ret.epsilon);
		}
	...
	fn triangulate(mut self) -> Vec<Vector3<i32>>
	{
		...
		{
			Self::triangulate_poly(start, &mut self.polygon, &self.polygon_range, &mut self.triangles, self.epsilon);
		}
	...
	fn loop_verts(mut first: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, mut func: impl FnMut(usize, &mut Vec<Vert>)) -> Option<usize>
	{
		let mut v = first;
		loop
		{
	...
	fn clip_ear(ear: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, triangles: &mut Vec<Vector3<i32>>)
	{
		...
		if left_mesh != self_mesh &&
				self_mesh != right_mesh &&
				right_mesh != left_mesh
		{
	...
	fn clip_if_degenerate(ear: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, triangles: &mut Vec<Vector3<i32>>, epsilon: f64)
	{
		let ear_ref = &polygon[ear];
		...
		if ear_ref.is_short(epsilon) ||
				(ccw(ear_ref.left().pos, ear_ref.pos, ear_ref.right().pos, epsilon) == 0 &&
				(ear_ref.left().pos - ear_ref.pos).dot(&(ear_ref.right().pos - ear_ref.pos)) > 0.0)
		{
	...
	fn initialize(&mut self, polys: &PolygonsIdx) -> Vec<usize>
	{
		...
		if self.epsilon < 0.0 { self.epsilon = self.bbox.scale() * K_PRECISION; }
	...
	fn find_start(&mut self, first: usize)
	{
		...
		{
			self.holes.insert_sorted_by_key(start, |&hole|
					Reverse(OrderedF64(self.polygon[hole].pos.x))); //descending pos.x
	...
	)
	{
		...
		let mut check_edge = |edge: usize, polygon: &mut Vec<Vert>|
		{
			...
			if x.is_finite() && start_ref.inside_edge(edge, epsilon, true) &&
					(connector.as_ref().map_or(true, |&connector|
					ccw(Point2::new(x, start_ref.pos.y), polygon[connector].pos, polygon[connector].right().pos,
					epsilon) == 1 ||
					(if polygon[connector].pos.y < edge.pos.y
					{ edge.inside_edge(&polygon[connector], epsilon, false) }
					else { !polygon[connector].inside_edge(edge, epsilon, false) })))
			{
		...
		let connector = Self::find_closer_bridge(start, connector.unwrap(), polygon, polygon_range, outers, epsilon);
	...
	fn find_closer_bridge(start: usize, edge: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, outers: &Vec<usize>, epsilon: f64) -> usize
	{
		let start_ref = &polygon[start];
		...
		let mut check_vert = |vert: usize, polygon: &mut Vec<Vert>|
		{
			...
			if vert.pos.x > start_ref.pos.x - epsilon &&
					...
					(inside > 0 || (inside == 0 && vert.pos.x < connector_ref.pos.x &&
					vert.pos.y * above_f64 < connector_ref.pos.y * above_f64)) &&
					vert.inside_edge(edge_ref, epsilon, true) && vert.is_reflexive(epsilon)
			{
	...
	fn join_polygons(start: usize, connector: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, triangles: &mut Vec<Vector3<i32>>, epsilon: f64)
	{
	...
	fn process_ear(v: usize, collider: &IdxCollider, ears_queue: &mut VecDeque<usize>, polygon: &mut Vec<Vert>, epsilon: f64)
	{
		if polygon[v].ear
		{
		...
		if polygon[v].is_short(epsilon)
		{
			...
			polygon[v].ear = true;
			ears_queue.insert_sorted_by_key(v, |&ear|
					OrderedF64(polygon[ear].cost)); //ascending cost
		}
	...
	fn triangulate_poly(start: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>, triangles: &mut Vec<Vector3<i32>>, epsilon: f64)
	{
...
impl Vert
{
	...
	fn interp_y2x(&self, start: Point2<f64>, on_top: i32, epsilon: f64) -> f64
	{
		...
		else if self.pos.y < start.y - epsilon
		{
			...
			{
				self.pos.x + (start.y - self.pos.y) * (self.right().pos.x - self.pos.x) /
	...
	{
		let cost = self.signed_dist(v, self.right_dir, epsilon).min(
				self.signed_dist(v, self.left().right_dir, epsilon));
	...
	fn ear_cost(&self, epsilon: f64, collider: &IdxCollider, polygon: &Vec<Vert>) -> f64
	{
		...
		let mut ear_box = Rect::new(center.coords.add_scalar(-radius).into(),
		...
		query_2d_tree(&collider.points, ear_box, |point|
		{
			...
			if !EarClip::clipped(test) && test.mesh_idx != self.mesh_idx &&
					test.mesh_idx != lid &&
					test.mesh_idx != rid
			{
				...
				if cost > total_cost { total_cost = cost; }
			}
		});
...
{
	if allow_convex && is_convex(polys, epsilon) //fast path
	{
