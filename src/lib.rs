...
impl MeshBool {
	...
	pub fn set_properties(
		...
		if num_prop == 0 {
			...
			// TODO: if prop_func.is_some() then sequential, else do parallel
			for tri in 0..self.num_tri() {
				for i in 0..3 {
					...
					if let Some(prop_func) = prop_func {
						prop_func(
							&mut meshbool_impl.properties
								[(num_prop * prop_vert) as usize..num_prop as usize],
							meshbool_impl.vert_pos[vert as usize],
							&old_properties[old_num_prop * prop_vert as usize..num_prop as usize],
						);
					} else {
						// Note: keeping this as it was from Manifold, but
						// don't we already set everything to 0.0?
						for p in 0..num_prop {
							meshbool_impl.properties[(num_prop * prop_vert + p) as usize] = 0.0;
						}
					}
				}
			}
