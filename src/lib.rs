...
use crate::common::{LossyFloat, LossyInt, Polygons};
...
impl<A: Copy, I: LossyInt> MeshGLP<A, I> {
	...
	pub fn num_tri(&self) -> I {
		I::lossy_from(self.tri_verts.len() / 3)
	}
}
...
impl MeshBool {
	...
	fn get_mesh_gl_impl<F: LossyFloat, I: LossyInt>(
		meshbool_impl: &MeshBoolImpl,
		normal_idx: i32,
	) -> MeshGLP<F, I> {
		...
		let out_num_prop: I = I::lossy_from(3 + num_prop);
		let tolerance = F::lossy_from(
			meshbool_impl
				.tolerance
		...
		for tri in 0..num_tri {
			...
			for i in 0..3 {
				tri_verts[3 * tri + i] =
					I::lossy_from(meshbool_impl.halfedge[3 * old_tri + i].start_vert);
			}
		...
		let mut vert_properties: Vec<F> =
			Vec::with_capacity(num_vert * (out_num_prop.lossy_as_usize()));
		...
		for run in 0..run_original_id.len() {
			for tri in
				(run_index[run].lossy_as_usize() / 3)..run_index[run + 1].lossy_as_usize() / 3
			{
				let tri = tri as usize;
				for i in 0..3 {
					let prop =
						meshbool_impl.halfedge[3 * (tri_new2old[tri] as usize) + i].prop_vert;
					let vert: usize = tri_verts[3 * tri + i].lossy_into();
					...
					let idx = vert_properties.len() / (out_num_prop.lossy_as_usize());
					...
					if update_normals {
						...
						let start = vert_properties.len() - (out_num_prop.lossy_as_usize());
						for i in 0..3 {
							normal[i] = vert_properties
								[((start + 3 + i) as i32 + normal_idx) as usize]
								.lossy_into();
						}
