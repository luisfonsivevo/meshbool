...
use crate::shared::{next_halfedge, Halfedge};
...
impl<'a> CheckHalfedges<'a>
{
	fn call(&self, edge: usize) -> bool
	{
		...
		if halfedge.start_vert == -1 && halfedge.end_vert == -1 &&
				halfedge.paired_halfedge == -1
		{
			return true;
		}
		...
		if halfedge.paired_halfedge == -1 { return false; }
...
impl Impl
{
	...
	*/
	...
	{
		if self.halfedge.len() == 0 { return true; }
		if self.halfedge.len() % 3 != 0 { return false; }
		(0..self.halfedge.len()).all(
				|edge| CheckHalfedges { halfedges: &self.halfedge }.call(edge))
	}
	...
	{
		if !self.is_manifold() { return false; }
	...
	{
		self.bbox.min = self.vert_pos.iter().fold(Point3::new(f64::INFINITY, f64::INFINITY, f64::INFINITY), |a, &b|
		{
			if a.x.is_nan() { return b; }
			if b.x.is_nan() { return a; }
			a.inf(&b)
		});
		
		self.bbox.max = self.vert_pos.iter().fold(Point3::new(f64::NEG_INFINITY, f64::NEG_INFINITY, f64::NEG_INFINITY), |a, &b|
		{
			if a.x.is_nan() { return b; }
			if b.x.is_nan() { return a; }
			a.sup(&b)
